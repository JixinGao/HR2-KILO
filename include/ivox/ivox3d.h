...
class IVox {
   public:
    ...
    bool GetGroundClosestPoint(const PointType& pt, PointVector& closest_pt, std::vector<float> & point_distance, int max_num = 5, double max_range = 5.0);

    bool GetGroundClosestPoint(const PointType& pt, PointVector& closest_pt, std::vector<float> & point_distance, const double &threshold, int max_num = 5, double max_range = 5.0);
...
template <int dim, IVoxNodeType node_type, typename PointType>
bool IVox<dim, node_type, PointType>::GetGroundClosestPoint(const PointType& pt, PointVector& closest_pt, std::vector<float> &point_distance, int max_num,
                                                      double max_range) {
    std::vector<DistPoint> candidates;
    candidates.reserve(max_num * nearby_grids_.size());
...
// #define INNER_TIMER
    ...
    for (const KeyType& delta : nearby_grids_) {
        ...
        if (iter != grids_map_.end()) {
            ...
            auto t1 = std::chrono::high_resolution_clock::now();
#endif
            auto tmp = iter->second->second.KNNGroundPointByCondition(candidates, pt, max_num, max_range);
#ifdef INNER_TIMER
            auto t2 = std::chrono::high_resolution_clock::now();
            auto knn = std::chrono::duration_cast<std::chrono::nanoseconds>(t2 - t1).count();
            stats["knn"].emplace_back(knn);
    ...
    for (auto& it : candidates) {
        closest_pt.emplace_back(it.Get());
        point_distance.emplace_back(it.dist);
    }
    return closest_pt.empty() == false;
}

template <int dim, IVoxNodeType node_type, typename PointType>
bool IVox<dim, node_type, PointType>::GetGroundClosestPoint(const PointType& pt, PointVector& closest_pt, std::vector<float> &point_distance, const double& threshold, 
...
// #define INNER_TIMER
    ...
    closest_pt.clear();
    for (auto& it : candidates) {
        if(it.Get().z > pt.z-abs(pt.z*threshold) && it.Get().z < pt.z+abs(pt.z*threshold)){
            closest_pt.emplace_back(it.Get());
            point_distance.emplace_back(it.dist);
        }
    }
